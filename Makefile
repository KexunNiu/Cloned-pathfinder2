...
# adds proxy aliases
hosts: 
	echo "127.0.0.1       platform.pathfinder.test api.platform.pathfinder.test mail.platform.pathfinder.test" | sudo tee -a /etc/hosts

# runs container
up:
	docker-compose up -d
	@echo "New dev URL is http://platform.pathfinder.test"
...
unit-f:
	docker-compose exec frontend yarn testWatch
unit-b:
	docker-compose exec backend python3 manage.py test . $(ARG) --keepdb
# --parallel --keepdb
