...
function OpportunityView({jobData, setReRender}) {
  ...
  return (
    ...
    <div className="max-w-sm  p-6 bg-white border border-sky-200 rounded-lg shadow-xl justify-center dark:bg-white  dark:border-sky-200">
      ...
      <button  className="relative inline-flex items-center justify-center p-0.5 mb-2 mr-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-red-200 via-red-300 to-yellow-200 group-hover:from-red-200 group-hover:via-red-300 group-hover:to-yellow-200 dark:text-black dark:hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-red-100 dark:focus:ring-red-400" 
        onClick={(e) => {
          ...
          del('api/opportunity/' + jobData.job.id).then((data) => {
    ...
    <img className='w-2/3 h-64 ml-10' src={jobData.job.opportunity_picture ? host+jobData.job.opportunity_picture : '../../defaultOpp.png'}/>
        <h3 className="mb-2 font-bold tracking-tight text-gray-500 dark:text-gray">Company: {jobData.job.company.name}</h3>
        <h4 className="mb-2 font-bold tracking-tight text-gray-900 dark:text-black">Name: {jobData.job.job_title}</h4>
        
        <h5 className="mb-2 font-bold tracking-tight text-gray-900 dark:text-black">Deadline: {formatDate(jobData.job.deadline)}</h5>
        <h4 className="mb-2 font-bold tracking-tight text-gray-500 dark:text-gray">Application #: {jobData.applicants.length}</h4>
       
        <time className="mb-1 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">Date Posted:{formatDate(jobData.job.date_posted)}</time>
        
    </a>
    <p className="mb-3 font-normal text-gray-700 dark:text-gray-400"></p>
    <Link href={{
      pathname:'/OpportunityEdit',
      query:jobData.job,
      }}> 
      <button className="relative inline-flex items-center justify-center p-0.5 mb-2 mr-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-red-200 via-red-300 to-yellow-200 group-hover:from-red-200 group-hover:via-red-300 group-hover:to-yellow-200 dark:text-black dark:hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-red-100 dark:focus:ring-red-400">
      ...
      <div
        ...
        <div className="relative w-full my-6 mx-auto max-w-3xl">
          ...
          <div className="border-0 h-screen overflow-y-scroll rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <p className="font-bold my-4 text-black text-lg leading-relaxed">Skills:</p>
           
              <p className="my-4 text-slate-700 text-lg leading-relaxed">
                  {jobData.job.job_skills}
              </p>
            ...
            <p className="my-4 text-slate-500 text-lg leading-relaxed">Applicants:
                 <h4>{jobData.applicants.map((item, index)=>{
                    return(
                            <>
                            <div className="p-1">
